...
angular.module("vdvcApp")
    .directive('mention', ['mentionUtil', '$document', '$compile', '$log', '$timeout','_',
        function (mentionUtil, $document, $compile, $log, $timeout,_) {
        return {
            restrict: 'A',
            scope: {
                macros: '=mentionMacros',
                search: '&mentionSearch',
                select: '&mentionSelect',
                items: '=mentionItems',
                mentionUsers:'&mentionUsers',
                typedTerm: '=mentionTypedTerm',
                altId: '=mentionId',
                iframeElement: '=mentionIframeElement',
                requireLeadingSpace: '=mentionRequireLeadingSpace',
                selectNotFound: '=mentionSelectNotFound',
                trimTerm: '=mentionTrimTerm',
                ngModel: '='
            },
            controller: function($scope, $timeout, $attrs) {
                ...
                $scope.bridgeSearch = function(termString) {
                    var searchFn = $attrs.mentionSearch ? $scope.search : $scope.defaultSearch;
                    searchFn({
                        term: termString
                    });
                };
                ...
                $scope.bridgeSelect = function(itemVar) {
                    var selectFn = $attrs.mentionSelect ? $scope.select : $scope.defaultSelect;
                    return selectFn({
                        item: itemVar
                    });
                };
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentionUtil.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                ...
                $scope.replaceMacro = function(macro, hasTrailingSpace) {
                    if (!hasTrailingSpace) {
                        ...
                        $scope.timer = $timeout(function() {
                            mentionUtil.replaceMacroText($scope.context(), $scope.targetElement,
                        ...
                        mentionUtil.replaceMacroText($scope.context(), $scope.targetElement, $scope.targetElementPath,
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentionItems) {
                    scope.localItems = [];
                    scope.parentScope = scope;
                    var itemsRef = attrs.mentionSearch ? ' mention-items="items"' : ' mention-items="localItems"';

                    scope.defaultTriggerChar = attrs.mentionTriggerChar ? scope.$eval(attrs.mentionTriggerChar) : '@';

                    var html = '<mention-menu' +
                        ' mention-search="bridgeSearch(term)"' +
                        ' mention-select="bridgeSelect(item)"' +
                        itemsRef;

                    if (attrs.mentionTemplateUrl) {
                        html = html + ' mention-template-url="' + attrs.mentionTemplateUrl + '"';
                    }
                    html = html + ' mention-trigger-char="\'' + scope.defaultTriggerChar + '\'"' +
                        ' mention-parent-scope="parentScope"' +
                        'style="display:none;"/>';
                    ...
                    function (newValue) {
                        ...
                        if (scope.triggerCharSet === undefined) {
                            $log.error('Error, no mention-items attribute was provided, ' +
                                'and no separate mention-menus were specified.  Nothing to do.');
                            return;
                        }
                        ...
                        var re = mentionUtil.getEmailRegex(scope.triggerCharSet);
                        var tre = mentionUtil.gettriggersRegex(scope.triggerCharSet);
                        var users = _.uniq(newValue.match(re));
                        
                        users = _.map(users, function(user){ 
                        	var rep = new RegExp("^"+tre.source);
                        	return user.replace(rep, ''); 
                        });
                        
                        if(scope.mentionUsers) {
                        	 scope.mentionUsers({
                             	users : users,
                             	id: scope.altId
                             });
                        ...
                        var mentionInfo = mentionUtil.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            ...
                            )
                        {
                            ...
                            var macroMatchInfo = mentionUtil.getMacroMatch(scope.context(), scope.macros);
    ...
    .directive('mentionMenu', ['mentionUtil', '$rootScope', '$log', '$window', '$document',
        function (mentionUtil, $rootScope, $log, $window, $document) {
        return {
            restrict: 'E',
            scope: {
                search: '&mentionSearch',
                select: '&mentionSelect',
                items: '=mentionItems',
                triggerChar: '=mentionTriggerChar',
                forElem: '=mentionFor',
                parentScope: '=mentionParentScope'
            },
            templateUrl: function(tElement, tAttrs) {
                return tAttrs.mentionTemplateUrl !== undefined ? tAttrs.mentionTemplateUrl : 'mention-menu.tpl.html';
            },
            ...
            link: function (scope, element) {
               // element[0].parentNode.removeChild(element[0]);
                ...
                if (scope.parentScope) {
                    ...
                    if (!scope.forElem) {
                        $log.error('mention-menu requires a target element in tbe mention-for attribute');
                        return;
                    }
                    if (!scope.triggerChar) {
                        $log.error('mention-menu requires a trigger char');
                        return;
                    }
                ...
                angular.element($window).bind(
                    'resize', function () {
                        if (scope.isVisible()) {
                            ...
                            mentionUtil.popUnderMention(scope.parentMentio.context(),
                ...
                scope.adjustScroll = function (direction) {
                    ...
                    var menuItem = (menuEl.querySelector('[mention-menu-item].active') || 
                        menuEl.querySelector('[data-mention-menu-item].active'));
    ...
    .directive('mentionMenuItem', function () {
        return {
            restrict: 'A',
            scope: {
    ...
    .filter('mentionHighlight', function() {
