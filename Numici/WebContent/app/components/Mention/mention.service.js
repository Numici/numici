...
angular.module("vdvcApp")
    .factory('mentionUtil', ["$window", "$location", "$anchorScroll", "$timeout","_",function ($window, $location, $anchorScroll, $timeout,_) {
        ...
        function popUnderMention (ctx, triggerCharSet, selectionEl, requireLeadingSpace) {
            ...
            var mW = selectionEl.width();
            var mH = selectionEl.height();
            ...
            if (mentionInfo !== undefined) {
                ...
                if((mW+coordinates.left) > wW) {
                	coordinates.left -= (mW+coordinates.left-wW+30);
                }
                
                if((mH+coordinates.top) > wH) {
                	coordinates.top -= (coordinates.top+30-mH);
                }
        ...
        function getTriggerInfo (ctx, triggerCharSet, requireLeadingSpace, menuAlreadyActive, hasTrailingSpace) {
            ...
            if (effectiveRange !== undefined && effectiveRange !== null) {
                ...
                triggerCharSet.forEach(function(c) {
                    ...
                    var triggersRegex = gettriggersRegex(c,true);
                    if(triggersRegex) {
                    	
                    	var idx = effectiveRange.regexLastIndexOf(triggersRegex)+1; //triggersRegex.lastIndex + space;
        ...
        function gettriggersRegex(triggers,leadingSpace) {
        	if( !_.isEmpty(triggers)) {
        		var triggersRegex = '';
        		if(_.isArray(triggers) ) {
   	       		 	triggersRegex = triggers.join('|').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        		}
        		
        		if(_.isString(triggers)) {
        			triggersRegex = triggers.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        		}
        		
        		if(leadingSpace) {
	       		 	return new RegExp("\\s("+triggersRegex+")");
	       		} else {
	       		 	return new RegExp("("+triggersRegex+")");
	       		}
        	}
        	
        }
        
        function getEmailRegex(triggers) {
        	 var triggersRegex = gettriggersRegex(triggers);
        	if(triggersRegex) {
        		return new RegExp(triggersRegex.source+"([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\\.[a-zA-Z0-9_-]+)","gi");
        	} else {
        		return new RegExp("([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\\.[a-zA-Z0-9_-]+)","gi");
        	}
        }
        ...
        return {
            ...
            gettriggersRegex: gettriggersRegex,
