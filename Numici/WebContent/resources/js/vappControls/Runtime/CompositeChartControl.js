var CompositeChartControl = ChartControl.extend({
	...
	getChartConfig : function () {
		...
		if ( xAxis.length > 0 ) {
			//config.axis.x.categories=xAxis;
		...
		if (configData && configData.Measures && configData.Measures.length > 0 ) {
			$.each(configData.Measures,function (i,obj) {
				...
				if (stack !== "NONE" && cluster !== "NONE") {
					var obj = c.getTriDimData(dim,cluster,stack,mes,xAxis);
					...
					var obj = c.getBiDimData(dim,stack,mes,xAxis);
					config.data.groups.push(obj.ids);
					c.columns = c.columns.concat(obj["data"]);
					c.setYaxisNCahrtType(yAxisType,chart,obj);

				} else if (cluster !== "NONE"  ) {
					var obj = c.getBiDimData(dim,cluster,mes,xAxis);
					c.columns = c.columns.concat(obj["data"]);
					c.setYaxisNCahrtType(yAxisType,chart,obj);
				} else {
					var dataId = mes;
					...
					var groupdata=c.parseToData(dataId,xAxis,group);
	...
	applyFilters : function(msr) {
		var c = this;
		var allFilters = c.getAppliedFilters(msr);
		var blkName = c.getBlockNameFromDim(msr);
		if(!$.isEmptyObject(allFilters)) {
			$.each(allFilters,function(key,val) {
	...
	updateChartData : function(msr) {
		
		var c = this;
		var xAxis = c.getXAxis();
		var configData = c.getCustomDesignerData();
		if (configData && configData.Measures && configData.Measures.length > 0 ) {
			$.each(configData.Measures,function (i,obj) {
...
var refconfig = {
	    "bindto": "#CompositeChart1 .ctrl-content",
	    "padding": {
	      "top": 10,
	      "right": 80,
	      "bottom": 10,
