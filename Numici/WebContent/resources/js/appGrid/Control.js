...
_.extend(Control.prototype, {
	...
	getJson: function (options) {
		...
		this.json["controlType"] = this.controlType || this.vappgenAttrs.name;
	...
	removeFilter: function (filter) {
		var c = this;
		var parent = c.getParent();
		var index =  parent.associatedFilters ? parent.associatedFilters.indexOf(filter) : -1;
		if (c.designerData && c.designerData.associatedCtrls && c.designerData.associatedCtrls.length > 0) {
			
			var filterableIds = c.designerData.associatedCtrls;
			$.each(filterableIds,function(i,id){
				var filterable = parent.getChildControlById(id);
				if (typeof filterable.resetFilters == "function") {
					filterable.resetFilters();
				}
				
				if (filterable && filterable.designerData && filterable.designerData.filterControlId) {
					delete filterable.designerData.filterControlId;
				}
			});
		}
		
		if (index != -1)
			parent.associatedFilters.splice(index, 1);
	},
	
	
	initVappControl: function (context, type, position, dimension) {		
	    ...
	    var grid = context.appGrid;
		var pos, size;	
	
	    if (position) {
	    	pos = {x: position.x, y: position.y};
	    	if (dimension) {
	    		size = {width: dimension.w, height: dimension.h};
	    	} else {
	    		size = {width: attrs.w, height: attrs.h};
	    	}
	    } else {
	    	
	    	if (typeof attrs.x !="undefined" && typeof attrs.y !="undefined") {
	    		pos = {x: attrs.x, y: attrs.y};
		    	size = {width: attrs.w, height: attrs.h};
	    	} else {
	    		var co = grid.get_cell_from_pixel({"top":attrs.topInPixels,"left":attrs.leftInPixels});
	    		pos = {x: co.x, y: co.y};
	    		size = {width: parseInt(attrs.width/8.333333), height: parseInt(attrs.heightInPixels/60)};
	    	}
	    	
	    }
		
		grid.add_widget(div, pos.x, pos.y, size.width, size.height);		
	   
		if (ctrl.controlType == "GroupControl") {
			grid.locked(div,true);			 	
		} 
	...
	*/
	...
	getDimensionInputs : function(model,dimension) {
		...
		$.each(model, function(index, value) {
			...
			if (typeof value[dimension] !== "undefined"  && $.inArray(value[dimension], set) == -1 ) {
	...
	initFilterControl: function () {
		...
		if (this.getControlType() == "CompositeFilter") {
			...
			if (this.getVappgenAttributes() && this.getVappgenAttributes().filterableControl && this.getAttributes().filterable){
	...
	getWrkSpaceDim : function(block,dimension) {
		var c = this, dim = null;
		if ( _.isArray(c.wrkSpaceDims) ) {
			$.each(c.wrkSpaceDims,function(i,d){
				if (d.blockName == block && d.name == dimension.name) {
					dim = d;
				}
			});
		}
	...
	hasPermission : function(block,dimName,dimVal) {
		...
		if( dimPerms && dimPerms.length > 0){
			...
			$.each(dimPerms,function(i,v){
				if($.isArray(v.permissions)) {
					...
					} else if(v.block === block && v.dimension === dimName && v.value == dimVal){
						if($.isArray(v.permissions) && v.permissions.length > 0) {
							status = true;
							v.isEditable = false;
							if(_.contains(v.permissions,"Edit")) {
								v.isEditable = true;
							}
	...
	getWrkSpaceDimValuesByPermissions : function(block,dimension) {
		 
		var c = this;
		var dimPerms = c.getPermissions();
		var values=[];
		var wrkSpaceDim = c.getWrkSpaceDim(block,dimension);
		if(c.mode != "design" && !_.isEmpty(wrkSpaceDim) ) {
			
			var dimValues = _.sortBy(wrkSpaceDim.values, 'idx');
			
			if (_.isArray(dimValues)) {
				
				$.each(dimValues,function(i,val) {
					var hasPerms = false;
					if(!status) {
						hasPerms = c.hasPermission(block,wrkSpaceDim.name,val.stringValue);
					} else {
						hasPerms = status;
					}
					
					if ( hasPerms){
						if (c.controlType == "Layout") {
							var isFiltered = c.isFiltered("dimension",block+"."+wrkSpaceDim.name,val.stringValue);
							if (isFiltered) {
								values.push(val.stringValue);
							}
						} else {
							values.push(val.stringValue);
						}
				...
				if (wrkSpaceDim.needAggregate && c.hasPermission(block,wrkSpaceDim.name,"*")) {
					values.push("*");
				}
			}
			...
			var agg = null;
			var key = block+"."+dimension.name;
			$.each(dimension.values,function(i,v){
				var val = v.trim();
				if(val == "*") {
					agg = val;
				} else if(c.isFiltered("dimension",key,val)) {
					values.push(val);
				} 
			});
			if(agg) {
