...
var LayoutControl = VidiViciControl.extend({
	...
	getCoordinates : function(data ,cell) {
		var c = this;
		var coord = [];
		if (c.wrkSpaceDims) {
			var dims = _.sortBy(c.wrkSpaceDims,"idx");
			$.each(dims,function(i,v){
				if(v.blockName == data["BlockName"] ) {
					coord.push(data[v.name]);
				}
			});
		}
		
		return coord.length > 0 ? coord : null;
	},
	
	getCellKey : function(data ,cell) {
		var c = this;
		var coords = c.getCoordinates(data,cell);
		
		
		if (coords) {
			coords.push($(cell).data('msrObj').name);
			var postdata = {};
				postdata["blockName"] = data["BlockName"];
				postdata["coordinates"] = coords;
			
			Util.ajaxPost('layout/cellKey/create',postdata,function(result) {
				
			    if (!result.Status) {
				   Util.showMessage(result.Message);
				} else {
					var node = result.Data;
					cell.attr("nodeid",result.Data);
					c.editedCells[node] = data;
				}
			});
		}
	},
	...
	eanbleAnnotations : function() {
		...
		$(document).on("click",parent+" .fa-comment",function(e) {
			...
			if (nodeId) {
				...
				annotationMgr.resourceId = c.workspaceId;
	...
	addEvents : function(){
		...
		if (c.selector) {
			...
			$(document).off("click",parent+ " .fa-history").on("click",parent+ " .fa-history",function(e) {
				...
				var nodeId = element.attr("nodeid");
				if (nodeId) {
					historyManager.show(nodeId,c.workspaceId);
				}
	...
	hasEditPermission : function(data) {
		
		var c = this,status = false;
		var dimPerms = c.getPermissions();
		
		if( dimPerms && dimPerms.length > 0 && !_.isEmpty(data)){
			var agg = null;
			var blk = data.BlockName;
			data = _.omit(data,["children","BlockName"]);
			$.each(dimPerms,function(i,v){
				if($.isArray(v.permissions)) {
					if (v.block == null && v.dimension == null) {
						if(_.contains(v.permissions,"AllCells")) {
							status = true;
							return false;
						}
					} else {
						$.each(data,function(key,val){
							var temp = _.findWhere(dimPerms, {"block": blk, "value": val,"dimension" : key});
							if(!_.isEmpty(temp) && _.isBoolean(temp.isEditable)) {
								status = temp.isEditable;
								if(!temp.isEditable) {
									return false;
								} 
							}
	...
	resetBlockDef : function() {
		
		var c = this;
		var blockdef=c.getBlockDef();
		var tempBlkdef = $.extend(true,{}, blockdef);
		if (c.designerData && c.designerData.appliedFilters && c.designerData.appliedFilters.DimensionFilters) {
			var dimFltrs = c.designerData.appliedFilters.DimensionFilters;
			$.each(dimFltrs,function(i,v){
				$.each(tempBlkdef,function(j,blk){
					$.each(blk.dimensions,function(k,dim){
						var key = blk.name+"."+dim.name;
						if (v.filterKye == key) {
							var flag = false;
							if(c.hasAggregate(dim.values)) {
								flag = true;
							}
							dim.values = v.values.slice(0,v.values.length);
							if (flag) {
								dim.values.push("*");
							}
							return false;
	...
	isFiltered : function(type,key,val) {
		
		var c = this,status = true;
		if (c.appliedFilters) {
			if (c.appliedFilters.DimensionFilters && type == "dimension") {
			...
			if (c.appliedFilters.MeasureFilters && type == "measure") {
				var msrFilters = c.appliedFilters.MeasureFilters;
				status = (_.contains(msrFilters,key));
			}
		}
		return status;
	},
		...
		if($.isArray(value)){
			$.each(value,function(i,v){
				...
				if(config === "values"){
					...
					var status = true;
					var msr = c.layoutBuilder.find(" .blockfields").find('li[role="measure"][block="'+v.blockName+'"][msr-name="'+v.name+'"]');
					var clonemsr=$(msr).clone();
					var measure = $(msr).data();
					var key = v.blockName+"."+measure.msrObj.name;
					status = c.isFiltered("measure",key);
					/*if((mode === "render" || mode === "compare") && measure.msrObj){
						var key = v.blockName+"."+measure.msrObj.name;
						status = c.isFiltered("measure",key);
					}*/
					if (status) {
						$(clonemsr).data($(msr).data());
						$(clonemsr).text(v.blockName+"."+v.name);
						$(clonemsr).attr("data-id",$(clonemsr).attr("id"));
						$(clonemsr).removeAttr("id");
						c.layoutBuilder.find(' div[data-holder="'+config+'"]').append(clonemsr);
						if(mode === "Launch"){
							$(msr).siblings('input[type="checkbox"]').prop("disabled",false);
							$(msr).siblings('input[type="checkbox"]').prop("checked",true);
							$(msr).parent("li").removeClass("disable");
							var measures = $(msr).closest(".root").find("li.disable");
							$.each(measures,function(i,v){
				...
				if(config === "rows" || config === "columns"){
					...
					c.getWrkSpaceDimValuesByPermissions(v.blockName,dimension.dimObj);
	...
	process : function(mode) {
		...
		if(typeof designerData !== "undefined" && !jQuery.isEmptyObject(designerData)){
			if (designerData.appliedFilters) {
				c.appliedFilters = designerData.appliedFilters;
			}
	...
	updateData : function() {
		...
		if(screen) {
			...
			if (!$.isEmptyObject(screen.aggXfilterObjs) ){
				c.aggXfilterObjs = screen.getAggXfilterObj();
			}
			if(!$.isEmptyObject(screen.aggDataSets)){
	...
	applyFilters : function() {
		...
		if(!$.isEmptyObject(allFilters)) {
			if ($.isArray(allFilters.DimensionFilters)) {
				...
				$.each(dimFltrs,function(i,v){
					var val = v.values.slice(0,v.values.length);
					var dimension = c.createDimensions(v.filterKye);
					var aggDimension = c.createAggDimensions(v.filterKye);
					c.filter(val,dimension);
					val.push("*");
					c.filter(val,aggDimension);
					if (i == dimFltrs.length-1) {
						var blockName = c.getBlockNameFromDim(v.filterKye);
						c.filteredData[blockName] = dimension.top(Infinity);
						c.filteredAggData[blockName] = aggDimension.top(Infinity);
						c.colOreFltrDta[blockName] = dimension.top(Infinity);
						c.colOreAggFltrDta[blockName]= aggDimension.top(Infinity);
						console.log(JSON.stringify(c.filteredAggData[blockName]));
	...
	getFilteredData : function(criteria,cellType) {
		...
		if (cellType === "aggregate") {
			...
			blockData = c.filteredAggData[blockName];
			if ( typeof blockData == "undefined" ) {
				blockData = c.getAggDataSet(blockName);
			}
		}  
		if (cellType === "normal") {
			blockData = c.filteredData[blockName];
			if ( typeof blockData == "undefined" ) {
	...
	showLayoutData : function() {
		...
		$.each(cellNodes,function(i,v){
			...
			if (root && msr) {
				...
				var isEditable = c.hasEditPermission(root); 
				if (!isEditable) {
					c.lockCell($(v));
				}
	...
	setColumnOrientedData : function() {
		...
		if (c.layoutBuilder) {
			...
			$.each(values,function(index,object){
				...
				data["lable"] = mes;
				data["key"] = $(object).attr("block")+"."+$(object).attr("msr-name");
				data["block"] = block;
				data["type"] = "measure";
				data["msrObj"] = msrObj;
	...
	NoOfRowsPerBlock : function(block) {
		...
		if(!$.isEmptyObject(c.dataSets)) {
			...
			if (c.colOreFltrDta[block]) {
				blkdata = c.colOreFltrDta[block];
			}
			if (blkdata && $.isArray(blkdata)) {
				rowCount = blkdata.length;
			}
	...
	findColOreFilteredRec : function(block,msr,rec) {
		...
		if ($.isArray(msrRecs) && msrRecs.length > 0) {
	...
	renderRows : function(block,columns) {
		...
		blkdata = c.filterBySortOrder(blkdata,block);
	...
	createInterLeavedGrid : function() {
		...
		if (rowdata.length > 0 ) {
			TotalNoOfRows=rowdata.length+columnHeadingLevels+1;
		} else {
			TotalNoOfRows=2*values.length+columnHeadingLevels+1;
		}
		//var extrows = c.calculateExtraRows(TotalNoOfRows);
	...
	filterBySortOrder : function(blkdata,block) {
		var c = this,data = [];
		var sortOrder = c.sortOrder;
		var filters = c.appliedFilters;
		if (blkdata && c.sortOrder && $.isArray(c.sortOrder) && $.isArray(c.coldimOrder) ) {
		...
		return (data.length > 0) ? data : blkdata;
	},
