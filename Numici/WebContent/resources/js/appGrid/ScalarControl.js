...
;(function(window){
	...
	window.ScalarControl = VidiViciControl.extend({
		...
		getScalarByName : function(name) {
			...
			if(window.modelInfo && window.modelInfo.scalars) {
				scalars = modelInfo.scalars;
			}
			if (scalars ) {
				$.each(scalars,function(i,sclr){
					if(sclr.scalarName == name || sclr.name == name) {
						scalar = sclr;
					}
				});
			}
			...
			if ( config) {
				...
				$.each(config,function(key,cfg){
					...
					if (sclr) {
						...
						switch(cfg["type"]) {
						case "TextBox" :
							var input = $('<input type="text" style="width:100%;" disabled>');
							if (c.mode == "run") {
								var input = $('<input type="text" style="width:100%;">');
							}
							input.val(sclr.value);
							input.data("scalar",sclr);
							td1.text(cfg.label || sclr.scalarName || sclr.name);
							td2.append(input);
							break;
						case "Slider" :
							...
							sliderVal.html((sclr.value || (!isNaN(min) ? min : 0)));
							td2.append(slider);
							var min = parseFloat(cfg.min);
							var max = parseFloat(cfg.max);
							
							slider.slider({
								value : sclr.value || (!isNaN(min) ? min : 0),
								disabled: (c.mode == "design") ? true : false,
								min : !isNaN(min) ? min : 0,
		...
		getHtmlConfigMarkup : function (scalars, inpConfig) {
			...
			for (var i=0; scalars && i<scalars.length; i++){
				var name = scalars[i].scalarName || scalars[i].name; 	
			...
			if (isExist.length == 0) {
				$(cofigUi).dialog({
					...
					open : function (event,ui) {
						...
						$(this).empty();
				    	$(this).append(message);
				    	$(this).find(".scalarInputs,.textInputs").hide();
					},
					buttons : {
						"Save" : function(event) {
