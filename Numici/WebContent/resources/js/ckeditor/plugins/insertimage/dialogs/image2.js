...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		uploadUrlHboxId = CKEDITOR.tools.getNextId(),
	...
	function toggleFileUploadVbox(value) {
		
		var fileUploadVbox = doc.find( '.uploadContainer' );
		if(value != "" || value) {
			try{
				fileUploadVbox.$[0].style.display='none';
			} catch(e){}
		} else {
			try{
				fileUploadVbox.$[0].style.display='block';
			} catch(e){}
		}
	}
	
	function insertLoadingHtml() {
		var loadingHtml = "<div id='loading-wrap' style='position: absolute;height: 100% !important;background: transparent;position: absolute;width: 100%;height: 15px;left: 0;top: 0;z-index: 1049;background-color: #ccc;opacity: 0.4;display: none;'>\
								<div id='loading-html' style='position: absolute;border: 5px solid #f3f3f3;border-radius: 50%;border-top: 5px solid #069;width: 15px;height: 15px;-webkit-animation: spin 2s linear infinite;animation: spin 2s linear infinite;margin: auto;top: calc(50% - 12.5px);left: calc(50% - 12.5px);'>\
							</div>\
						</div>";
		var imageDailogElement = $(".cke_dialog_page_contents");
		imageDailogElement.prepend(loadingHtml);
	}
	...
	return {
		...
		onShow: function() {
			...
			toggleFileUploadVbox(widget.data.src);
						...
						children: [
							{
								...
								onChange: function( ) {
									...
									try {
										$("#loading-wrap").show();
										var ubtn = doc.find('.uploadButton');
										ubtn.getItem(0).fire('click');
							...
							},
							{
								...
								filebrowser: {
									 		action : 'QuickUpload', // required
									  		target : 'Upload:src', // required
									  		onSelect : function( fileUrl, errorMessage ) {
									  			if(!errorMessage) {
									  				//editor.showNotification( "Image Uploaded successfully", 'info' );
									  				var uRl = doc.find('.uploadedUrl');
													try {
														uRl.getItem(0).setStyle("display","block");
													} catch(e) {
