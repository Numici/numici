...
var finEdgarDataImportWizard2 = function(modelId,menuRole){
	...
	if (this.menuRole == "balanceSheetHistory2") {
		this.forEdgarDataGroup = "BalanceSheetConsolidated";
		this.importFor = "EdgarBalanceSheet2";
	} else if (this.menuRole == "incomeStatementHistory2") {
		this.forEdgarDataGroup = "IncomeStatementConsolidated"
		this.importFor = "EdgarIncomeStatement2";
	} else if (this.menuRole == "cashFlowStatementHistory2") {
		this.forEdgarDataGroup = "CashFlowStatementConsolidated"
		this.importFor = "EdgarCashFlowStatement2";
	} else if (this.menuRole == "valuationRatiosHistory2") {
		this.forEdgarDataGroup = "ValuationRatiosMini"
		this.importFor = "EdgarValuationRatios2";
	} else if (this.menuRole == "profitabilityRatiosHistory2") {
		this.forEdgarDataGroup = "ProfitabilityRatiosMini"
		this.importFor = "EdgarProfitabilityRatios2";
	}
	...
	this.createFinEdgarDataImprtDlg = function () {
		...
		$(mapDlg).dialog({
				...
				create: function (event, ui){
					...
					//obj2['mappingFor']="EdgarBalanceSheet";
    ...
    var getFinImportJSON = function (mappings, processType, dataGroup, statementFrequency, numOfYears, tickerSymbol, newBlocks) {
    	var json = {};
    	
    	json['modelId'] = importDlg.modelId;
    	//json['edgarDataGroup'] = edgarDataGroup;
    	json['dataGroup'] = importDlg.forEdgarDataGroup;
    	json['statementFrequency'] = statementFrequency;
    	json['numOfYears'] = numOfYears;
    	var tickerSymbols="";
  ...
  //  	blkMap['blockType']='New';
   ...
   // 	blkMap['blockType']='Model';
    ...
    var nextMappingDimensionsCB = function (dlg) {
    	...
    	importDlg.json = getFinImportJSON (importDlg.mappingInputs, "Validate", "BalanceSheetConsolidated", importDlg.blockInputs.statementFrequency, importDlg.blockInputs.numOfYears, importDlg.blockInputs.tickers, importDlg.newBlocks);
    ...
    var createMappingDialog = function () {
		...
		$(dlg).dialog({
				...
				open : function (event,ui) {
					...
					inputObj.dataGroup = importDlg.forEdgarDataGroup;
					inputObj.statementFrequency = importDlg.blockInputs.statementFrequency;
					inputObj.numOfYears = importDlg.blockInputs.numOfYears;
					
					//Util.ajaxPost ("import/csv/columns", inputObj, function(result){
					Util.ajaxPost ("importEdgarFinData2/edgarCols", inputObj, function(result){

			    		if (!result.Status) {
							Util.showMessage (result.Message);
						} else {
							importDlg.columns = result.EdgarDataGroupColumns;
    ...
    var saveMappings = function () {
        ...
        if(mapFile && mapFile.trim() != "") {
        	...
        	//mapJson['mappingFor']="EdgarBalanceSheet";
...
/*    	if (json['newBlocks']){
		...
		Util.ajaxPost ("importEdgarFinData2/edgarFinData", json, function(result){
    		if (!result.Status) {
    ...
    var createPreviewDialog = function () {
		...
		$(dlg).dialog({
				...
				open : function (event,ui) {
					...
					Util.ajaxPost ("importEdgarFinData2/edgarFinData", importDlg.json, function(result){
						if (!result.Status) {
...
var createSelectModelBlocksHTML = function (){
	...
	var periodDuration = 	{	"Annual": "ANN",
			"Quarterly" : "QTR"
		};
	var retriveInformation = 	{	"1 Year": 1,
			"2 Years": 2,
			"3 Years": 3,
			"4 Years": 4,
			"5 Years": 5
...
//		markup += '<br>';
	...
	markup += '<td>Statement Duration</td>';

	markup += '<td>';
	markup += '<select class="blockProps docInputs" dataAttr="statementFrequency" style="width: 100%;">';
	$.each(periodDuration, function(key, value) {
		if ((!$.isEmptyObject(importDlg.refMap) && importDlg.refMap.statementFrequency == value) || (key == "Annual")){
	...
	markup += '<td>Retrive Information For</td>';

	markup += '<td>';
	markup += '<select class="blockProps docInputs" dataAttr="numOfYears" style="width: 100%;">';
	$.each(retriveInformation, function(key, value) {
		if ((!$.isEmptyObject(importDlg.refMap) && importDlg.refMap.numOfYears == value) || (key == "1")){
			markup += '<option value="'+value+'" selected>'+key+'</option>';
		} else {
			markup += '<option value="'+value+'">'+key+'</option>';
		}
	...
	markup +='<td style="width:50%;"><div style="height: 140px; overflow: auto;">';
	...
	$(blkDlg).dialog({
			...
			create: function (event, ui){
				...
				$.when(
						...
						Util.ajaxSyncGet ("financial/data/EdgarOnline", function(result){
							if (!result.Status) {
								Util.showMessage (result.Message);
							} else {
