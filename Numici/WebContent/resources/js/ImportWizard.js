...
var ImportWizard = function() {
	...
	var createSelectMappingsHTML = function () {
    	...
    	markup += '<td>';
    	if (importDlg.modelMatchedMappings && !importDlg.modelMatchedMappings.length)
    		markup += '<input type="radio" name="selMaps" value="mapFiles" disabled>Mappings matching file name';
    	else {
    		markup += '<input type="radio" name="selMaps" value="mapFiles" checked>Mappings matching file name';
    		checked = (importDlg.modelMatchedMappings && !importDlg.modelMatchedMappings.length) ? false : true;
    	}
    	markup += '</td>';
    	if (checked) {
	    	markup += '<td>';   	
	...
	var nextSelectedMappingsCB = function () {
    	...
    	importDlg.savedMap = {};
		var importBlockDlg = importDlg.dialogs["BrowseBlock"];    	
    	if (importBlockDlg){
	...
	var createSelectMappingsDialog = function () {
		...
		var title = "ImportWizard: SelectMapType - "+importDlg.originalFile;
    ...
    var createSelectModelBlocksHTML = function (){
    	...
    	var delimiters = 	{	"Comma": ",",
    							"Semicolon" : ";",
    							"Colon": ":"
    						};
    	var lineSeparators = 	{	"New Line": "\n",
									"Pipe" : "\\|"
								};
    ...
    //		markup += '<br>';
    	...
    	markup += '<td>Field Separator</td>';
   
    	markup += '<td>';   	
    	markup += '<select class="blockProps docInputs" dataAttr="delimiter">';
    	$.each(delimiters, function(key, value) {
    		if ((!$.isEmptyObject(importDlg.refMap) && importDlg.refMap.delimiter == value) || (key == "Comma")){
    	...
    	markup += '<td>Record Separator</td>';
   
    	markup += '<td>';   	
    	markup += '<select class="blockProps docInputs" dataAttr="separator">';
    	$.each(lineSeparators, function(key, value) {
    		if ((!$.isEmptyObject(importDlg.refMap) && importDlg.refMap.lineSep == value) || (key == "Comma")){
    			markup += '<option value="'+value+'" selected>'+key+'</option>';
    		} else {
    			markup += '<option value="'+value+'">'+key+'</option>';
    		}
    ...
    var validateModelBlocksMapUserInputs = function (blk, inputs){
		...
		for (var key in inputs) {
			...
			if (err.status && inputs[key].CSV == "Select CSV Column"){
				if (type != "measure"){					
					err.status = false;			
			...
			if (type == "measure" && inputs[key].CSV != "Select CSV Column")
    ...
    var getImportJSON = function (mappings, processType, fileName, newBlocks) {
    	var json = {};
    	
    	json['fileName'] = fileName;
    	json['modelId'] = importDlg.modelId;
    	json['delimiter'] = importDlg.blockInputs.delimiter;
    	json['lineSep'] = importDlg.blockInputs.separator;
  ...
  //  	blkMap['blockType']='New';
   ...
   // 	blkMap['blockType']='Model';
    ...
    removeInvalidMeasures = function (map){
    	for (key in map){
    		if (map[key].type == 'measure' && map[key].CSV == "Select CSV Column"){
    			delete map[key];
    		} 
    	}
    };
    ...
    var nextMappingDimensionsCB = function (dlg) {
    	...
    	importDlg.json = getImportJSON (importDlg.mappingInputs, "Validate", importDlg.importedFile, importDlg.newBlocks);
		...
		$(dlg).dialog({
				...
				open : function (event,ui) {
					...
					Util.ajaxPost ("import/csv/columns", inputObj, function(result){
			    		if (!result.Status) {
							...
							importDlg.columns = result.CsvColumns;
				...
				buttons : {
					...
					"Prev" : function(event) {
						$(this).dialog("close");	
						importDlg.buttonMode = "Prev";
						importDlg.dialogs["BrowseBlock"].dialog("open");
					},
					"Next" : function(event) {
						Util.showSpinner();
						importDlg.buttonMode = "Next";
    	...
    	for (var key in importDlg.blockInputs.blocks){
    		...
    		if (type == "model") {
		    	...
		    	markup += '<div><table border="0" style="width:100%">\
		    		<tr> <th> Block Fields </th> <th> CSV Element </th> </tr></table>\
		    	...
		    	var columns = importDlg.columns;
		       	...
		       	for (var i=0; importDlg.modelBlks && i<importDlg.modelBlks.length; i++){
		    		...
		    		for (var k=0; blk.dimensions && k < blk.dimensions.length; k++){
			    		...
			    		for (var j=0; j<columns.length; j++){
			    			if (!$.isEmptyObject(importDlg.refMap) && refBlk) {
			    				if (refBlk.blockMapping && refBlk.blockMapping[dim.name]['CSV'] == columns[j])
		    		...
		    		for (var k=0; blk.measures && k < blk.measures.length; k++){
			    		...
			    		for (var j=0; j<columns.length; j++){
			    			if (!$.isEmptyObject(importDlg.refMap) && refBlk) {
			    				if (refBlk.blockMapping && refBlk.blockMapping[ms.name]['CSV'] == columns[j])
			    					markup += '<option selected="selected" value="'+columns[j]+'">'+columns[j]+'</option>';
			    				else
			    					markup += '<option value="'+columns[j]+'">'+columns[j]+'</option>';
			    			} else {
    		...
    		//	var formats = ["F1", "F2"];
		    	...
		    	markup += '<div><table border="0" style="width:100%" class="mapNewBlocks">\
		    		<tr> <th>CSV Element</th> <th>Role</th> <th>Name</th> <th>Type</th>  <th>Format</th> </tr> </table>\
...
/*			markup += '<td style="text-align:right;width:50%">';
			...
			markup += '</td>';
			markup += '<td style="text-align:left;width:50%;">';
    ...
    var isInvalidRowsPresent = function (result) {   
    	for (var i=0; result.CsvRows && i<result.CsvRows.length; i++){
    		var blk = result.CsvRows[i];
    ...
    var createPreviewDialog = function () {
		...
		var title = "ImportWizard: Preview - "+importDlg.originalFile;
